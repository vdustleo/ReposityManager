window.StorageOutRecord = (function($, module) {
    ...
    function init() {
        // 修改导航
        $("#navi > ul").empty().html("<li>首页</li> -><li>出库记录</li>");
    ...
    function initStorageGrid() {
        _$grid.jqGrid(_.extend({
            url : "storage/listOutItem.action",
            datatype : "json",
            mtype : "post",
            height : 300,
            width : 1188,
            colNames : [ '序号', '果农信息', '存储编号', '苹果类型', '苹果尺寸', '仓库编号', 
                         '货物数量', '存储单价', '存储总价', '出库时间', '隔档编号', 'id'],
            colModel : [ {
                name : '---',
                index : '---',
                width : 60,
    ...
    function printOutRecord(){
        ...
        // 打印入库次数
        LODOP.ADD_PRINT_TEXT(orginTop, orginLeft + 530, width, height, "出库次数:");
        LODOP.ADD_PRINT_TEXT(orginTop, orginLeft + 590, width, height, inTimes);
        
        // 打印入库数量
        LODOP.ADD_PRINT_TEXT(orginTop, orginLeft + 620, width, height, "出库数量:");
        ...
        LODOP.ADD_PRINT_TEXT(orginTop, orginLeft + 530, width, height, "出库时间");
    ...
    return module;
}($, window.StorageOutRecord||{}));

$(function() {
    StorageOutRecord.init();
});